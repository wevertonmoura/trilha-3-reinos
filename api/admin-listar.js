...
export default async function handler(req, res) {
  ...
  // O servidor verifica se a senha que você digitou bate com o cofre do Vercel
  if (senha !== process.env.VITE_SENHA_ADMIN) return res.status(401).json({ error: 'Acesso negado' });

  const { data, error } = await supabase.from('inscricao_trilha').select('*').order('created_at', { ascending: false });
