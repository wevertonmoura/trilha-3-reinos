...
export default async function handler(req, res) {
  ...
  // 🛡️ CORREÇÃO DE BUG: Apontando para a tabela correta lista_espera_trilha
  const { error } = await supabase.from('lista_espera_trilha').delete().eq('id', id);
  
  if (error) {
    console.error("[ERRO EXCLUIR ESPERA]:", error.message);
